...
namespace ApiGenerator.NSwagWrapper.Commands.Document
{
    [Command(Name = "run", Description = "Executes an .nswag file (with pre-processing). If 'input' is not specified then all *.nswag files and the nswag.json file is executed.")]
    public class PreProcessedExecuteDocumentCommand : IConsoleCommand
    {
        ...
        private async Task ExecuteDocumentAsync(IConsoleHost host, string filePath)
        {
            ...
            var document = await NSwagDocumentWithPreProcessing.LoadWithTransformationsAsync(filePath, Variables);
